...
const Expanded = ( { mainImage, imagesArray, slice, changeMainImage, changeSlice, toggleExpanded } ) => {
  ...
  return (
    <MainImage currentImage={mainImage ? mainImage : ''}>
      <ImageCarousel>
        {iconArray.length ? iconArray.length > 7 ?
          <CarouselArrow onClick={() => updateCarousel('up') }>
            <div className='arrow up'
                 style={{ marginLeft: 0.75 + 'rem' }}></div>
          </CarouselArrow>
          : <div style={{ width: 3 + 'rem', height: 1 + 'rem'}}></div> : null}
        <div>
          {iconArray.length ? iconArray.slice(slice[0], slice[1]).map((icon, i) => {
                return (
                  <div key={icon.photo.thumbnail_url}>
                    <ImageIcon onClick={() => updateCurrentImage(icon.photo)}>{icon.value}</ImageIcon>
                    <ImageNoUnderline />
      ...
      <div  style={{ width: 1 + 'rem',
                     ...
                     top: 0 + '%',
                     left: 95 + '%'}}
            onClick={() => toggleExpanded(false)}>X
      </div>
    </MainImage>
  );
};
